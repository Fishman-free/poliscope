...
EpistemoBrain 本身是\textbf{无投票权的组织脑}：它负责调度、记录与组装，不是第八名科学家，也不代表任何学术立场。七席共享同一套运行框架（\code{deliberation.py} 中唯一的 \code{GatewayDeliberator} 类服务全部席位）与同一个工具缓存，既满足「不复制七套业务代码、不部署七个服务」，又保住席位之间的独立性。
...
研究者离开页面不会卡住任务：宽限期默认 300 秒（环境变量全名见附录 A.4，下限 30 秒），届满自动以「无干预」回到队列续跑。实现横跨状态枚举、\code{CouncilCheckpoint.guidance} 字段、预览与引导两个 API 端点（\code{council-preview}、\code{council-guidance}）、CLI 子命令与前端 \code{CheckpointGate.tsx}。
