...
\code{DELETE /api/tasks/\{id\}} 物理删除任务及全部子记录，这是权限模型唯一的例外（迁移 0019 单独授予应用角色 DELETE），界面提供两段式确认与「清空全部」。测试分三层：无外部依赖的单元测试、必须真实连接 PostgreSQL 的集成测试（无数据库时显式 skip 并说明，绝不静默全绿）、Golden 因果蕴含用例与发布前 E2E 门禁。当前共 \textbf{1210 项自动化测试}；业务规则、状态机与图约束遵循先测后写，修复缺陷必须附带回归测试。
