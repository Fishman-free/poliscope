...
\medskip
Poliscope 面向计算社会科学争议问题（首版领域：数字行为、社交媒体与心理健康），给出的回答是一套完整的开源研究系统。它的方法基座是 \textbf{MemoBrain} 的单智能体执行记忆；本项目把它推广为面向多智能体的 \textbf{EpistemoBrain}，再适配进「七人科学议会 + 双图证据治理」的决策框架。也就是说，本文档讲的是三件事：\textbf{谁在研究}（第 4 章）、\textbf{靠什么记得}（第 2.1、5 章）、\textbf{什么算证据}（第 6、7 章）。下文说明这套系统的机制、工程实现、评测证据与设计边界。

\subsection*{\sffamily\color{accentdk}四条核心创新}

\begin{enumerate}[leftmargin=1.6em, itemsep=4pt, topsep=4pt]
\item \textbf{多智能体执行记忆（EpistemoBrain）。}把 MemoBrain 为\textbf{单个}智能体设计的执行记忆推广到多智能体，由三部分构成：\textbf{集体执行记忆}——记忆的所有者从「写它的人」变成「没有写它的同伴」；\textbf{异议保真记忆操作}——\code{Quarantine} 隔离保留、\code{Dialectical Fold} 辩证折叠、\code{Fork} 认知分叉、\code{Resurrect} 假设复活、\code{Perspective Recall} 角色化召回；\textbf{盲点驱动调度}——由证据图暴露的缺口决定下一步查什么、由谁查。这条推广暴露了单智能体场景从不存在的硬约束：记忆由私有变为公共，于是压缩不再只是内部优化，而是一次必须留痕、且必须与正式结论分图存放的编辑。它是后面所有治理机制的前置条件（第 2、5 章）。
...
\item \textbf{可证伪的实现与评测。}系统以模块化单体完整落地（14 个内部包、4 个应用、43 个数据库迁移、1210 项自动化测试），并配套 ForesightBlindspot 时间切片评测协议：五条能力递增基线、六项单机制消融，真实模型上的对照结果与其边界一并公开。
\end{enumerate}
...
\thd{自动化测试} & 1210 项（单元 / 集成 / Golden / E2E / 评测） & \thd{许可证} & MIT \\
\end{tabularx}
\end{tcolorbox}
