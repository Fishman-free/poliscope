...
\item \textbf{可证伪的实现与评测。}系统以模块化单体完整落地（14 个内部包、4 个应用、43 个数据库迁移、1206 项自动化测试），并配套 ForesightBlindspot 时间切片评测协议：五条能力递增基线、六项单机制消融，真实模型上的对照结果与其边界一并公开。
\end{enumerate}
...
\thd{自动化测试} & 1206 项（单元 / 集成 / Golden / E2E / 评测） & \thd{许可证} & MIT \\
\end{tabularx}
\end{tcolorbox}
